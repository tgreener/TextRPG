...
class EntityFactory {
    static func createEntityWithPortal(to room: Room) -> Entity {
        ...
        return Entity(portalComponent: portal, pickUpComponent: nil, inventoryComponent: nil)
    }
    
    static func createRock() -> Entity {
        let rockPickUp : PickUp = PickUpItem(name: "rock", description: "a really nice rock", duration: 1.0)
        return Entity(portalComponent: nil, pickUpComponent: rockPickUp, inventoryComponent: nil)
    }
}
